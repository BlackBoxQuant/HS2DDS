...
def QUERY_SURFACE(img = np.ndarray, qNode = Node):
    global table
    bgrArray = []
    for x in range(len(qNode.firstName)):           #THIS IS ALL ABOUT THE FIRST NAME
        bgrArray.append(table[qNode.firstName[x]])
    print("looking for:", bgrArray)
    #EVENTUALLY WILL BE multithreaded search
    for x in range(surface):
        for y in range(surface):
            j=0
            while np.array_equal(img[x][y+j], bgrArray[j]):
                if j == (len(bgrArray)-1):
                    print("###########FULL HIT ON FIRST NAME############")
                    return   
                else:
                    j+=1
        y+=3
